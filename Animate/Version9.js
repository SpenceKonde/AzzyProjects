...
function handleCmd(pn,q,r) {
	try {
		...
		if (pn=="/save.cmd") {
    		...
    		r.write(JSON.stringify({"base":leds.tbuf,"twinkle":[leds.tm,leds.ta,leds.ti],"current":leds.tclb}));
    		return -1;
  		} else if (pn=="/off.cmd") {
    		if (leds.lastscene) {
...
leds.fbuf=new Uint8Array(numleds*4); //Buffer actually sent to the LEDs - 4 bytes per pixel. 
