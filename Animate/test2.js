...
leds.flip = function () {
  ...
  //console.log(tclb);
	for (var i=0;i<numleds;i++) {
		//var x = new Uint16Array([tclb[i*3]<<5,tclb[(i*3)+1]<<5,tclb[(i*3)+2]<<5]);
		var rch=gtab[tclb[i*3]
		var gch=gtab[tclb[i*3+1]
		var bch=gtab[tclb[i*3+2]
		//var x = new Uint16Array([gtab[tclb[i*3]],gtab[tclb[(i*3)+1]],gtab[tclb[(i*3)+2]]]);
		...
		this.fbuf[i*4]=(this.ison?(this.gdim[i]|224):224);
		this.fbuf[1+i*4]=(bch?Math.max(x[2]>>4,1):0);
		this.fbuf[2+i*4]=(gch?Math.max(x[2]>>4,1):0);
		this.fbuf[3+i*4]=(rch?Math.max(x[2]>>4,1):0)
